...
rl.on('line', line => {
  ...
  const targetQueue = input[0].split('');
  const curiQueue = input[1].split('');

  while (curiQueue.length) {
    if (curiQueue.shift() === targetQueue[0]) targetQueue.shift();
  }

  console.log(targetQueue.length ? 'NO' : 'YES');
});
