...
const solution = (board, moves) => {
  let result = 0;
  const boardStack = board.map((item, i, arr) => {
    const stack = [];
  ...
  for (let move of moves) {
    ...
    const item = boardStack[move - 1].pop();
    if (bucketStack[bucketStack.length - 1] === item) {
      bucketStack.pop();
      result += 2;
    } else {
      bucketStack.push(item);
    }
  }
